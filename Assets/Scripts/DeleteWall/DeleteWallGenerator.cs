...
public class DeleteWallGenerator : MonoBehaviour
{
    ...
    public void SetStraightMode(bool straight)
    {
        ...
        if (GeneratorUIController.Instance != null)
        {
            GeneratorUIController.Instance.UpdateModeStatus($"Wall Mode: {(isDeleteModeOn ? "ON" : "OFF")}\nType: {(straight ? "Straight Only" : "Free Draw")}");
        }
    }
    ...
    private GameObject InstantiateWall(Vector3 startPoint, Vector3 endPoint)
    {
        try
        {
            ...
            // Add the delete behavior script
            wall.AddComponent<DeleteWallBehavior>();
            
            // Add Rigidbody with disabled physics
            Rigidbody rb = wall.AddComponent<Rigidbody>();
            rb.useGravity = false;
            rb.isKinematic = true; // Prevents physics from affecting the wall
            ...
            // Destroy the wall after 0.2 seconds
            Destroy(wall, 0.2f);
